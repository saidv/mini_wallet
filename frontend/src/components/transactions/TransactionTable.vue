...
const getDirectionIcon = (transaction: Transaction) => {
  return getTransactionDirection(transaction) === 'sent' ? 'mdi-arrow-up' : 'mdi-arrow-down';
};

const getDirectionColor = (transaction: Transaction) => {
  return getTransactionDirection(transaction) === 'sent' ? 'error' : 'success';
};
  ...
  <v-table class="bordered-table" hover density="comfortable">
    <thead class="bg-containerBg">
      <tr>
        ...
        <th v-if="showActions" class="text-center text-caption font-weight-bold text-uppercase">
          Actions
        </th>
      </tr>
    </thead>
      ...
      <tr v-for="transaction in transactions" :key="transaction.uuid" class="py-4">
        ...
        <td class="text-body-2">
          <div>
            {{
              compact
                ? formatDateCompact(transaction.created_at)
        ...
        <td>
          ...
          <div v-else class="d-flex align-center">
            <v-avatar size="32" :color="getDirectionColor(transaction)" class="mr-3">
        ...
        <td class="text-center">
          <v-chip v-if="compact" variant="text" size="small" class="px-0">
            <v-avatar
              size="8"
          ...
          <v-chip v-else :color="getStatusColor(transaction.status)" size="small" variant="flat">
            {{ transaction.status }}
          </v-chip>
        </td>
        ...
        <td v-if="showActions" class="text-center">
          <v-btn icon size="small" variant="text" @click="copyToClipboard(transaction.uuid)">
            <v-icon size="20">mdi-content-copy</v-icon>
            <v-tooltip activator="parent" location="top"> Copy Transaction ID </v-tooltip>
          </v-btn>
        </td>
      </tr>
    </tbody>
