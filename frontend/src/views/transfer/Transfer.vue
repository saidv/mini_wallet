...
const amountRules = [
  ...
  (v: string) => Number(v) <= (authStore.currentBalanceInDollars - calculateCommission(Number(v))) || 'Insufficient balance including commission'
]
...
const validateReceiver = async () => {
  ...
  try {
    ...
    if (error.response?.data) {
      receiverError.value = error.response.data.message || 'User not found'
    } else {
                ...
                <v-card v-if="receiverUser" flat color="white" class="pa-4 mt-2">
                  <div class="d-flex align-center">
                    <v-avatar color="primary" size="48" class="mr-4">
                      <span class="text-h6 text-white">{{ receiverUser.name.charAt(0).toUpperCase() }}</span>
                    </v-avatar>
              ...
              <v-card v-if="amountInDollars && Number(amountInDollars) > 0" flat class="pa-4 mb-4">
                ...
                <v-list lines="two" class="bg-transparent">
                  <v-list-item>
                    ...
                    <template #append>
                      <span class="text-h6 font-weight-bold">${{ Number(amountInDollars).toFixed(2) }}</span>
                    </template>
                  </v-list-item>
                  ...
                  <v-list-item>
                    ...
                    <template #append>
                      <span class="text-h5 font-weight-bold text-error">${{ totalDebit.toFixed(2) }}</span>
                    </template>
                  </v-list-item>
                  
                  <v-list-item>
                    <v-list-item-title class="font-weight-bold">Remaining Balance</v-list-item-title>
                    <template #append>
                      <span 
